...
select_user() {
    ...
    local SHOW_LOCKED=$3
    mapfile -t raw_user_array < <(jq -r '.inbounds[1].settings.clients[].email' "$CONFIG_FILE" | grep -v "dummy" | sort)
